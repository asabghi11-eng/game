(function(){
  ...
  function tryJoinSlotsBatch(slotNums){
    ...
    slotNums.forEach(slotNum=>{
      ...
      try{
        ...
        console.error('quick play: failed to create Peer', e);
        return;
      }
      mmBatchPeers.push(p);
      p.on('open', ()=>{
      ...
      p.on('error', (err)=>{ mmLastErrorType = (err && err.type) || 'unknown'; console.error('quick play: scan peer error', err); }); // handled by the shared failTimeout above
    });
  }
  function tryClaimSlot(slotNum){
    ...
    try{
      ...
      quickStatus.textContent = 'خطا در اتصال به سرورِ آنلاین. اینترنتت رو چک کن و دوباره امتحان کن.';
      quickStatus.className = 'status-line err';
      quickSpinner.classList.add('hidden');
      quickPlayMode = false;
      refundStakeIfUnmatched();
    ...
    peer.on('open', ()=>{
      ...
      // Being a host means we're no longer scanning. If two players both
      // claim slots around the same time (their initial scans missed each
      // other), neither will EVER be found unless someone goes back to
      // scanning — this was the root cause of "waiting for opponent"
      // forever. So don't wait here indefinitely: give up this slot after a
      // short, randomized wait and resume scanning ourselves. The
      // randomization (vs. a fixed delay) keeps two simultaneous hosts from
      // flipping between "host" and "scan" in lockstep and missing each
      // other again.
      const hostWaitMs = MM_HOST_WAIT_MIN + Math.random() * (MM_HOST_WAIT_MAX - MM_HOST_WAIT_MIN);
    ...
    peer.on('error', (err)=>{
      ...
      if(peer){ try{ peer.destroy(); }catch(e){} peer = null; }
      if(mmCancelled) return;
      quickStatus.textContent = 'خطا: ' + mmLastErrorType + ' — در حال تلاشِ دوباره...';
      scanNextSlot();
    });
  }
