(function(){
  ...
  function scanNextSlot(){
    ...
    if(mmAttemptIdx >= mmSlotOrder.length-1){
      quickStatus.textContent = 'الان کسی آنلاین پیدا نشد. یکم دیگه امتحان کن یا با کدِ روم بازی کن.';
  ...
  // MM_CONNECT_TIMEOUT, we fall back to claiming the first slot in the batch
  // (same claim behavior as before, just reached ~MM_BATCH_SIZE times faster).
  function tryJoinSlotsBatch(slotNums){
    ...
    const failTimeout = setTimeout(()=>{
      ...
      settled = true;
      destroyBatchPeers(null);
      if(!mmCancelled) tryClaimSlot(slotNums[0]);
    }, MM_CONNECT_TIMEOUT);
  ...
  function attemptCreateTournament(){
    ...
    peer = new Peer(TOURNAMENT_PREFIX + code);
    peer.on('open', ()=>{
  ...
  shopBtnApp.addEventListener('click', openShop);
