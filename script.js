(function(){
  ...
  const ICE_SERVERS = [
    ...
    { urls: 'turn:free.expressturn.com:80?transport=tcp', username: '000000002100651466', credential: 'OTWoNw2HIhM1pz++89XdvfKoUOI=' },
    { urls: 'turn:free.expressturn.com:443?transport=tcp', username: '000000002100651466', credential: 'OTWoNw2HIhM1pz++89XdvfKoUOI=' },
  ];
  const PEER_OPTS = { config: { iceServers: ICE_SERVERS } };
  const ROOM_PREFIX = "mesir-azad-";
  const MM_PREFIX = "mesir-azad-mm-";
  const MM_SLOTS = 12; // fewer slots = much higher odds two players' batches overlap on the first try
  const MM_CONNECT_TIMEOUT = 3000;
  const MM_CLAIM_TIMEOUT = 3000; // claiming an id from the signaling server is normally near-instant; no need to wait long
  const MM_HOST_WAIT_MIN = 1000; // once we become a "host" slot, how long to wait for someone to find us
  const MM_HOST_WAIT_MAX = 2000; // before giving up and going back to scanning ourselves (randomized to avoid two hosts flipping in lockstep)
  const MM_TOTAL_TIMEOUT = 45000; // hard ceiling: after this, stop retrying and show a real error instead of spinning forever
  const MM_BATCH_SIZE = 4; // slots probed IN PARALLEL per round — this is what actually makes matching fast
  const STAKE_TIERS = [
  ...
  // In online games, the host (myRole===0) starts at the top of the board by
  // default. Rather than always showing player-1-at-top/player-2-at-bottom
  // the same way for both screens, we visually rotate the whole board 180°
  // on the host's screen so BOTH players always see themselves at the
  // bottom, like most games in this genre. Game logic/state is untouched —
  // this only affects rendering (draw()) and pointer-to-cell translation
  // (getPointer()).
  let boardFlipped = false;
  // Draws text/emoji upright even while the board is visually flipped, by
  // counter-rotating just for this one glyph.
  function fillTextUpright(text, x, y){
    if(!boardFlipped){ ctx.fillText(text, x, y); return; }
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(Math.PI);
  ...
  function openOnlineOverlay(){
    ...
    if(typeof Peer === 'undefined'){
      toast('⚠️ کتابخانه‌ی اتصالِ آنلاین لود نشد. اینترنتت رو چک کن و صفحه رو رفرش کن.');
    }
  }
  ...
  // Serverless matchmaking trick, "claim-first": for each well-known "slot" ID
  // we try to CLAIM it first — i.e. register ourselves as that peer ID. The
  // signaling server only lets one peer own an ID at a time, so this
  // resolves almost instantly:
  //   - claim succeeds  -> nobody was here, we become the host and wait.
  //   - claim fails with 'unavailable-id' -> someone is ALREADY hosting this
  //     slot, so we immediately connect to them as a client instead.
  // The key to making this fast: we don't try slots one at a time. Each
  // round we claim MM_BATCH_SIZE slots AT ONCE (in parallel). With few
  // enough total slots, two players' random batches are very likely to
  // overlap on the very first round, so matches typically resolve in a
  // couple of seconds instead of slowly stepping through the list in
  // lockstep. If a whole round comes up empty, we move to the next batch
  // of slots and try again, up to MM_TOTAL_TIMEOUT.
  ...
  function destroyBatchPeers(exceptPeer){
    mmBatchPeers.forEach(p=>{
      if(p && p!==exceptPeer){ try{ p.destroy(); }catch(e){} }
    });
    mmBatchPeers = exceptPeer ? [exceptPeer] : [];
  }
  ...
  function giveUpMatchmaking(){
    quickStatus.textContent = 'اتصال به سرورِ آنلاین برقرار نشد' + (mmLastErrorType ? (' (خطا: ' + mmLastErrorType + ')') : '') + '. اینترنتت رو چک کن یا چند لحظه‌ی دیگه دوباره امتحان کن.';
  ...
  function attemptBatch(){
    if(mmCancelled || mmMatched) return;
    if(Date.now() - mmStartedAt > MM_TOTAL_TIMEOUT){ giveUpMatchmaking(); return; }
    quickStatus.textContent = 'در حال جستجوی حریف...';
    quickStatus.className = 'status-line';
    quickSpinner.classList.remove('hidden');
    const start = (mmRoundIdx * MM_BATCH_SIZE) % mmSlotOrder.length;
    mmRoundIdx++;
    const batch = [];
    for(let i=0;i<MM_BATCH_SIZE;i++) batch.push(mmSlotOrder[(start+i) % mmSlotOrder.length]);
    mmPendingInBatch = batch.length;
    mmBatchPeers = [];
    batch.forEach(slotNum => attemptSlotInBatch(slotNum));
  }
  // Called whenever one slot's attempt (claim, or the join-fallback it led
  // to) finishes without a match. Once every slot in the current batch has
  // reported back empty-handed, we move on to the next batch of slots.
  function slotAttemptDone(){
    if(mmCancelled || mmMatched) return;
    mmPendingInBatch--;
    if(mmPendingInBatch <= 0) attemptBatch();
  }
  function matchFound(p, c, role){
    if(mmMatched){ try{ c.close(); }catch(e){} try{ p.destroy(); }catch(e){} return; }
    mmMatched = true;
    destroyBatchPeers(p); // another slot in this same batch may have also connected — drop it, we only need one
    peer = p;
    myRole = role;
    currentRoomCode = null;
    setupConnection(c);
  ...
  function attemptSlotInBatch(slotNum){
    if(mmCancelled || mmMatched) return;
    const slotId = MM_PREFIX + (quickPlayStake||0) + '-' + slotNum;
    let p;
    try{
      p = new Peer(slotId, PEER_OPTS);
    }catch(e){
      console.error('quick play: failed to create claim Peer', e);
      slotAttemptDone();
      return;
    }
    mmBatchPeers.push(p);
    ...
    p.on('open', ()=>{
      // Nobody was registered on this slot — we're the host now. Wait a
      // short while for someone else's claim attempt to bounce off us.
      if(settled) return;
      settled = true;
      clearTimeout(claimTimeout);
      if(mmCancelled || mmMatched){ try{ p.destroy(); }catch(e){} return; }
      quickStatus.textContent = 'منتظرِ یه حریفِ دیگه...';
      let matched = false;
      const hostWaitTimeout = setTimeout(()=>{
        if(matched || mmCancelled || mmMatched) return;
        try{ p.destroy(); }catch(e){}
        slotAttemptDone();
      }, MM_HOST_WAIT_MIN + Math.floor(Math.random() * (MM_HOST_WAIT_MAX - MM_HOST_WAIT_MIN)));
      p.on('connection', (c)=>{
        if(mmCancelled || mmMatched) return;
        matched = true;
        clearTimeout(hostWaitTimeout);
        c.on('open', ()=> matchFound(p, c, 0));
      });
    });
    p.on('error', (err)=>{
      if(settled) return;
      settled = true;
      clearTimeout(claimTimeout);
      mmLastErrorType = (err && err.type) || 'unknown';
      if(err && err.type === 'unavailable-id'){
        // Someone else already claimed this slot — they're the host, so
        // connect to them directly instead of waiting around.
        try{ p.destroy(); }catch(e){}
        if(mmCancelled || mmMatched) return;
        joinExistingSlotInBatch(slotNum);
        return;
      }
      console.error('quick play: claim peer error', err);
      try{ p.destroy(); }catch(e){}
      slotAttemptDone();
    });
  }
  function joinExistingSlotInBatch(slotNum){
    if(mmCancelled || mmMatched) return;
    const slotId = MM_PREFIX + (quickPlayStake||0) + '-' + slotNum;
    let p;
    try{
      ...
      console.error('quick play: failed to create client Peer', e);
      slotAttemptDone();
      return;
    }
    mmBatchPeers.push(p);
    ...
    p.on('open', ()=>{
      if(settled || mmCancelled || mmMatched) return;
      ...
      // per-connection error: leave it to failTimeout, which will move us to
      // the next batch — the host we tried to reach may have just left
      c.on('error', (err)=> console.error('quick play: connect error', err));
    });
    p.on('error', (err)=>{
      if(settled) return;
      settled = true;
      clearTimeout(failTimeout);
      mmLastErrorType = (err && err.type) || 'unknown';
      console.error('quick play: client peer error', err);
      try{ p.destroy(); }catch(e){}
      slotAttemptDone();
    });
  }
  ...
  function drawIceCells(){
    ...
    state.iceCells.forEach(key=>{
      ...
      ctx.restore();
      ctx.globalAlpha = .85;
      fillTextUpright('❄️', px+cell/2, py+cell/2);
      ctx.globalAlpha = 1;
    });
    ctx.restore();
  }
  ...
  function drawFragileOverlay(x,y,vertical,turnsLeft){
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    fillTextUpright(String(turnsLeft), cx, cy);
    ctx.restore();
  }
  ...
  function draw(now){
    if(!cell) return;
    const size = N*cell;
    boardFlipped = onlineMode && myRole === 0;
    ctx.clearRect(0,0,size,size);
    ctx.save();
    if(boardFlipped){ ctx.translate(size,size); ctx.rotate(Math.PI); }
  ...
  function drawItems(){
    ...
    keys.forEach(key=>{
      ...
      ctx.shadowBlur = 6;
      fillTextUpright(ITEM_ICON[state.items[key]] || '★', cx, cy);
    });
    ctx.restore();
  }
  ...
  function drawPlayer(gx, gy, idx){
    ...
    if(avEmoji){
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      fillTextUpright(avEmoji, cx, cy-r*0.15);
      ctx.restore();
    }
  }
