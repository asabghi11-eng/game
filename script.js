(function(){
  ...
  const MM_HOST_WAIT_MIN = 3000; // once we become a "host" slot, how long to wait for someone to find us
  const MM_HOST_WAIT_MAX = 6000; // before giving up and going back to scanning ourselves (randomized to avoid two hosts flipping in lockstep)
  ...
  // (in random order) we try to CLAIM it first — i.e. register ourselves as
  // that peer ID. The signaling server only lets one peer own an ID at a
  // time, so this resolves almost instantly (well under a second):
  ...
  // This is what makes two simultaneous "quick play" presses converge fast:
  // the server itself acts as the tie-breaker (exactly one of the two claims
  // can win), so whoever loses the claim race finds out almost instantly and
  // connects straight to the winner — no blind waiting on either side. And if
  // we end up as the host with nobody finding us, we don't wait forever: we
  // give up after MM_HOST_WAIT_MS and try again, so both sides keep cycling
  // through slots until they land on each other.
  ...
  // Moves on to the next slot in our shuffled order (wrapping around) after a
  // claim or a connection attempt didn't pan out. Slots are tiny, quick
  // attempts, so cycling through several of them still comfortably fits
  // inside a handful of seconds.
  function nextSlot(prevSlotNum){
    if(mmCancelled) return;
    if(Date.now() - mmStartedAt > MM_TOTAL_TIMEOUT){ giveUpMatchmaking(); return; }
    let idx = mmSlotOrder.indexOf(prevSlotNum);
    if(idx === -1) idx = mmAttemptIdx % mmSlotOrder.length;
    idx = (idx + 1) % mmSlotOrder.length;
    if(idx === 0) mmRoundsTried++;
    mmAttemptIdx++;
    quickStatus.textContent = 'در حال جستجوی حریف...';
    quickStatus.className = 'status-line';
    quickSpinner.classList.remove('hidden');
    attemptSlot(mmSlotOrder[idx]);
  }
  function attemptSlot(slotNum){
    if(mmCancelled) return;
    if(Date.now() - mmStartedAt > MM_TOTAL_TIMEOUT){ giveUpMatchmaking(); return; }
  ...
  function joinExistingSlot(slotNum){
    if(mmCancelled) return;
    const slotId = MM_PREFIX + (quickPlayStake||0) + '-' + slotNum;
    let p;
    try{
    ...
    const failTimeout = setTimeout(()=>{
      ...
      try{ p.destroy(); }catch(e){}
      if(peer===p) peer = null;
      if(!mmCancelled) nextSlot(slotNum);
    }, MM_CONNECT_TIMEOUT);
    p.on('open', ()=>{
      ...
      // the next slot — the host we tried to reach may have just left
